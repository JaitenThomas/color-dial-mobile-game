...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
    ...
    void Start() {
        ...
        //NEW
        startingPage = SaveManager.instance.state.indexOfSelectedWheel;
    ...
    private void LerpToPage(int aPageIndex) {
        ...
        _lerp = true;
        _currentPage = aPageIndex;
        OnSelected(); //NEW - set the correct button varibles
        CurrentPage(); // control what page we are on
    }
    ...
    public void CurrentPage()
    {
        ...
        if (_currentPage == 0)
        {
            StartCoroutine(ShopManager.instance.wheels[0].GetComponent<DisplayWheelController>().Rotate(3));
            
        }

        if (_currentPage == 1)
        {
            StartCoroutine(ShopManager.instance.wheels[1].GetComponent<DisplayWheelController>().Rotate(3));
        }

        if (_currentPage == 2)
        {
            StartCoroutine(ShopManager.instance.wheels[2].GetComponent<DisplayWheelController>().Rotate(3));
        }
        */
    ...
    public void OnSelected()
    {
        ...
        if (ShopManager.instance.itemsBroughtState[_currentPage] == false || ShopManager.instance.itemsBroughtState[_currentPage] == true)
        {
            ShopManager.instance.shopButtons[_currentPage].GetComponent<ShopItem>().CheckActive();
            ShopManager.instance.buyButton.transform.GetChild(0).GetComponent<Text>().text = ShopManager.instance.costs[_currentPage].ToString();
            ShopManager.instance.currentIndex = _currentPage;
        }

        if (ShopManager.instance.wheels[_currentPage].GetComponent<ShopItem>().rariety == ShopItem.Rariety.common)
        {
            rarietyText.color = Color.white;
            rarietyText.text = "Common";
        }

        else if (ShopManager.instance.wheels[_currentPage].GetComponent<ShopItem>().rariety == ShopItem.Rariety.rare)
        {
            rarietyText.color = new Color32(64, 64, 255, 255);
            rarietyText.text = "Rare";
        }
         
        else if (ShopManager.instance.wheels[_currentPage].GetComponent<ShopItem>().rariety == ShopItem.Rariety.epic)
        {
            rarietyText.color = new Color32(128, 64, 255, 255);
            rarietyText.text = "Epic";
        }

        else if (ShopManager.instance.wheels[_currentPage].GetComponent<ShopItem>().rariety == ShopItem.Rariety.legendary)
        {
            rarietyText.color = new Color32(255, 128, 64, 255);
            rarietyText.text = "Legendary";
        }

        else if (ShopManager.instance.wheels[_currentPage].GetComponent<ShopItem>().rariety == ShopItem.Rariety.mystery)
        {
            rarietyText.color = new Color32(128, 128, 128, 255);
            rarietyText.text = "Mystery";
        }
